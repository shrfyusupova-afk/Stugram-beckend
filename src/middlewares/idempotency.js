...
const idempotency = async (req, res, next) => {
  try {
    ...
    // Unlike rate limiting, idempotency protects against duplicate side
    // effects (double-created posts/comments/stories) -- if we can't
    // reliably check/record it, fail closed (surface an error) instead of
    // hanging or silently letting the request through unprotected.
    return next(error);
  }
};
